...
class DWT_3D(Module):
    ...
    def get_matrix(self):
        ...
        if torch.cuda.is_available():
            ...
            self.matrix_low_0 = torch.Tensor(matrix_h_0).to(mps_device)
            self.matrix_low_1 = torch.Tensor(matrix_h_1).to(mps_device)
            self.matrix_low_2 = torch.Tensor(matrix_h_2).to(mps_device)
            self.matrix_high_0 = torch.Tensor(matrix_g_0).to(mps_device)
            self.matrix_high_1 = torch.Tensor(matrix_g_1).to(mps_device)
            self.matrix_high_2 = torch.Tensor(matrix_g_2).to(mps_device)
        else:
            self.matrix_low_0 = torch.Tensor(matrix_h_0)
            self.matrix_low_1 = torch.Tensor(matrix_h_1)
