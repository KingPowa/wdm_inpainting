...
from .utils.nn_utils import conv_nd, linear, avg_pool_nd, zero_module, normalization, timestep_embedding, checkpoint
...
class UNetModel(LightningModule):
    ...
    def forward(self, x, timesteps, label_cond_dwt=None, y=None):
        """
        ...
        if self.use_conditional_model:
            h = th.cat((x, label_cond_dwt), dim=1)
        else:
            h = x
